...
namespace CustomBigInt
{
    public class CustomBigInt
    {
        ...
        public CustomBigInt(string number)
        {
            ...
            if (number.Contains(","))
            {
                _index = number.IndexOf(",", StringComparison.Ordinal);
                number = number.Remove(_index, 1);
            }
        ...
        {
            if (Equals(valueOne, valueTwo)) // check if the two numbers are equal
            {
                return true;
            }
        ...
        public static bool operator !=(CustomBigInt valueOne, CustomBigInt valueTwo)
        {
            if (!Equals(valueOne, valueTwo)) //check if the two numbers are different
            {
                return true;
            }
        ...
        public static CustomBigInt operator -(CustomBigInt valueOne, CustomBigInt valueTwo)
        {
            ...
            if (valueOneString.Length > valueTwoString.Length)
            {
                ...
                char[] valueOneCharArray = valueOneString.ToCharArray();
                char[] valueTwoCharArray = valueTwoString.ToCharArray();

                Array.Reverse(valueOneCharArray);
                Array.Reverse(valueTwoCharArray);

                string reverseValueOne = new string(valueOneCharArray);
                string reverseValueTwo = new string(valueTwoCharArray);


                for (int i = 0; i <= length - 1; i++)
                {
                    int valueOneInt = int.Parse(reverseValueOne[i].ToString());
                    int valueTwoInt = int.Parse(reverseValueTwo[i].ToString());

                    if (valueOneInt < valueTwoInt)
                    {
                        valueOneInt += 10;

                        minus = minus + Math.Pow(10, i + 1);
                        con = true;
                    }
            ...
            result = new string(reverseResult);
